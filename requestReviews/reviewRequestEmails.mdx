...
- Collect reviews from 5-10% of customers (vs <1% without asking)
...
<Steps>
  ...
  <Step title="Yuko Waits" icon="clock">
    Yuko waits the number of days you set (default: 7 days).
  ...
  <Step title="Customer Clicks & Reviews" icon="pen-to-square">
    Customer clicks button, lands on pre-filled review form, writes review.
...
<AccordionGroup>
  <Accordion title="Timing: When to Send" icon="calendar" defaultOpen>
    **Setting:** Delay (days)
...
<CardGroup cols={2}>
  ...
  <Card title="Build Trust" icon="shield-check">
    ...
    **From name:** Use "Sarah from [Store]" not "noreply@store.com"
  </Card>
...
<Steps titleSize="h3">
  <Step title="Configure Email Settings" icon="sliders">
    ...
    **Time:** 5 minutes
    **Location:** Reviews → Emails → Review Request Email
  </Step>
  ...
  <Step title="Customize with Shortcodes" icon="code">
    ...
    **Time:** 3 minutes
    **Use:** `{{customer_first_name}}` and `{{product_reviews_block}}`
  </Step>

  <Step title="Match Your Brand" icon="palette">
    Add your logo and colors to make emails professional.

    **Time:** 5 minutes

    → [Customize Email Branding](/reviewSettings/branding)
  </Step>
  ...
  <Step title="Send Test Email" icon="paper-plane">
    ...
    **Time:** 2 minutes
    **Action:** Click "Send Test Email" button at bottom of settings
  </Step>
...
<Note>
**Pro tip:** Start with a 7-day delay for most products. After a month, check your response rate. If it's low (<3%), try increasing the delay (customers may need more time) or adding a small incentive.
</Note>
