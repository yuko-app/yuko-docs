...
In addition, the upgrade event includes any **per-benefit reward codes** issued at upgrade — fields named `yuko_reward_1_*`, `yuko_reward_2_*`, etc. (depends on benefits configured for the new tier). Inspect the actual payload in Klaviyo's metric inspector for your specific store.
...
- **VIP Tier Upgrade** → Send a celebratory tier email with `yuko_benefits_unlocked` and `yuko_new_earning_rate`.
