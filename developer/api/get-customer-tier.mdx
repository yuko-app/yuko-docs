...
This endpoint requires the `tiers` scope with **Read Access**. Configure token scopes in Settings → Integrations → REST API.
...
```bash cURL
curl --request GET \
  --url "https://api.yukoapp.com/api/v1/public/customers/tiers?customer_id=9049402769586" \
...
const response = await fetch(
  `https://api.yukoapp.com/api/v1/public/customers/tiers?customer_id=${customerId}`,
  {
    method: "GET",
    headers: {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.yukoapp.com/api/v1/public/customers/tiers?customer_id={$customerId}");
