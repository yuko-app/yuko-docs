...
```bash cURL
curl --request GET \
  --url "https://api.yukoapp.com/v1/public/customers/9049402769586/rewards?status=active&per_page=20" \
...
const response = await fetch(
  `https://api.yukoapp.com/v1/public/customers/${customerId}/rewards?${params}`,
  {
    method: "GET",
    headers: {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.yukoapp.com/v1/public/customers/{$customerId}/rewards?{$query}");
